...
Проект развёрнут на отдельном production-стенде:

    https://z50128.adman.cloud

Демо позволяет проверить основной пользовательский сценарий без локального запуска проекта.
...
- realtime-обновление остатка и цены;
- создание заказа;
- резервирование товара на 5 минут;
- countdown до окончания резерва;
- обновление цены перед оплатой;
- тестовая оплата;
- автоматическая выдача цифрового ключа;
- просмотр выданного ключа;
- повторная обработка payment webhook без повторной выдачи.
...
2. Выбрать товар с доступным остатком.

3. Создать заказ.

4. Проверить таймер резерва.

5. Выполнить тестовую оплату.

6. После обработки заказа открыть страницу заказа и проверить выданный ключ.

### Realtime demo

Для проверки realtime можно открыть сайт в двух вкладках.

Изменение остатка или цены товара передаётся через Laravel Reverb и Laravel Echo без перезагрузки страницы.

Если товар заканчивается, кнопка покупки отключается во всех открытых вкладках.

### Production API

API доступен по адресу:

    https://z50128.adman.cloud/api

Например:

    GET https://z50128.adman.cloud/api/products

Production использует HTTPS и WebSocket через WSS.

Исходный код:

    https://github.com/51mpLeDev/game-keys

Демо работает на тестовых данных.
Платёжный provider и provider выдачи цифровых товаров являются mock/test реализациями.
Реальные денежные операции не выполняются.
...
Frontend успешно собирается командой:

    npm run build

Production-сборка выполняется в Node.js 22 Docker-контейнере.
